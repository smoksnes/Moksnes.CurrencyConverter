...
public class CurrencyServiceTests
{
    ...
    private FakeResponseHandler mockHandler;
    ...
    public CurrencyServiceTests()
    {
        ...
        {
            BaseUri = "http://local.host/api/endpoint",
            LatestWithBase = "{base}"
        };
        var options = Options.Create(config);
        mockHandler = new FakeResponseHandler();
        client = new HttpClient(mockHandler);
        httpClientFactory.CreateClient(CurrencyConfig.ClientName).Returns(client);
        service = new CurrencyService(httpClientFactory, options);
    }

    [Fact]
    public async Task GivenBaseCurrency_WhenGetCurrenciesShouldFetchRates()
    {
        var baseCurrency = WellKnownCurrencies.SEK;
        var jsonString =
        ...
        mockHandler.SetFakeResponse(new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(jsonString)
        });
        var result = await service.GetCurrenciesAsync(baseCurrency);
